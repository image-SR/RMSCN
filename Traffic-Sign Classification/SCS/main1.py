...
with open('/root/autodl-tmp/driver/train1.csv', 'r') as file:
    ...
    for row in reader:
        if int(row[2]) == 0:
            # image_sub_path1.append(os.path.join("/root/train_3",row[0].split("/")[5]+"/"+row[0].split("/")[6]))
            image_sub_path1.append(row[0])
            image_sub_label1.append(row[1])

image_sub_valid_path1 = []
image_sub_valid_label1 = []
with open('/root/autodl-tmp/driver/valid1.csv', 'r') as file:
    reader = csv.reader(file)
    for row in reader:
        if int(row[2]) == 0:
            # image_sub_valid_path1.append(os.path.join("/root/valid_3",row[0].split("/")[5]))
            image_sub_valid_path1.append(row[0])
            image_sub_valid_label1.append(row[1])
...
#训练
train_data_1 = MyDataset(image_sub_path1,image_sub_label1,transform=transform)
train_loader_1 = DataLoader(train_data_1, batch_size=32, shuffle=True, num_workers=8)
valid_data_1 = MyDataset(image_sub_valid_path1,image_sub_valid_label1,transform=transform)
valid_loader_1 = DataLoader(valid_data_1, batch_size=32, shuffle=True, num_workers=8)
#搭建模型
import torch
...
def train(model, device, train_loader,optimizer, EPOCHS):
    ...
    # torch.save(model.state_dict(), os.path.join("/root/autodl-tmp/driver/weight1", 'epoch_{}.pth'.format(epoch)))
# 定义测试过程
...
for epoch in range(1, EPOCHS + 1):
    train(model, DEVICE, train_loader_1, optimizer, epoch)
    avg_correct = test(model, DEVICE, valid_loader_1)
    if best_correct < avg_correct:
        best_epoch = epoch
        best_correct = avg_correct
        best_weight = copy.deepcopy(model.state_dict())
print('best_epoch {}, best_correct {:.3f}'.format(best_epoch, best_correct))
torch.save(best_weight,  os.path.join("/root/autodl-tmp/driver/weight1", 'best4.pth'))